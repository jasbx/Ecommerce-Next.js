...
const Header = () => {
  ...
  const getCartItem=()=>{
    CartApis.getCartItem(user.primaryEmailAddress.emailAddress).then(res=>{
        res?.data?.data.forEach(cartItem=>{
            setCart((oldCart)=>[
                ...oldCart,
                {
                    id: cartItem.id,
                    product: cartItem?.attributes?.product?.data[0],
                    ...
                    <div className="flex items-center lg:order-2">
                     {
                        ...
                        <Link href="/sign-up" className="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-4 lg:px-5 py-2 lg:py-2.5 mr-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">Register</Link>
                        
                        </>
                        :<div className='flex gap-5 items-center justify-content-around'>
                            <h2 className='flex gap-2'><ShoppingCart onClick={()=>setOpen(!openCart)}/>({cart?.length})</h2>
                            <UserButton />
                            {openCart &&<Cart/>}
                        </div>
                     }
                        <button onClick={()=>setMenu(!openMenu)} data-collapse-toggle="mobile-menu-2" type="button" className="inline-flex items-center p-2 ml-1 text-sm text-gray-500 rounded-lg  hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-gray-200 dark:text-gray-400 dark:hover:bg-gray-700 dark:focus:ring-gray-600" aria-controls="mobile-menu-2" aria-expanded="false">
